...
g_options = jacinle.FileOptions(__file__, use_self_mask=True, use_softmax_iota=True, use_mul_and=False, normalize=False, use_max_for_rel=False, normalize_iota=False)
...
def _do_apply_self_mask(m):
    ...
    if g_options.normalize:
        return m * (1 - self_mask) + (-1) * self_mask
    else:
...
class NCGeneralizedFOLExecutor(FunctionDomainExecutor):
    ...
    def greater_than(self, x: TensorValue, y: TensorValue) -> TensorValue:
        if g_options.normalize:
            rv = torch.sigmoid((x.tensor - y.tensor - 1 + 2 * self._count_margin) / self._count_tau)
        elif self.training:
    ...
    def equal(self, x: TensorValue, y: TensorValue) -> TensorValue:
        if g_options.normalize:
            rv = torch.sigmoid((2 * self._count_margin - (x.tensor - y.tensor).abs()) / (2 * self._count_margin) / self._count_tau)
        elif self.training:
    ...
    def _execute_inner(self, expr: E.Expression) -> TensorValue:
        if isinstance(expr, E.BoolExpression):
            ...
            if expr.bool_op is E.BoolOpType.AND:
                ...
                if g_options.use_mul_and:
                    result = torch.stack(expanded_tensors, dim=-1).prod(dim=-1)
                else:
                ...
                if g_options.normalize:
                    result = torch.ones_like(result) - result
                else:
            ...
            if expr.function.name in self.function_implementations:
                ...
                if len(args) == 1:
                    ...
                    if len(self.view_stack) > 0:
                        ...
                        if g_options.normalize:
                            ## normalize the grounding tensor by division of size of 'i'
                            grounding_tensor = grounding_tensor / obj_anchor.tensor.size(0)
                    else:
                        ...
                        if g_options.normalize:
                            ...
                            grounding_tensor = grounding_tensor - torch.diag_embed(torch.diagonal(grounding_tensor, dim1=-2, dim2=-1))
                ...
                for i, arg in enumerate(args):
                    if isinstance(arg, Variable):
                        ...
                        if not shift:
                            if g_options.use_max_for_rel:
                                grounding_tensor, _ = (grounding_tensor * jactorch.add_dim_as_except(arg.tensor, grounding_tensor, i)).max(i, keepdim=True)
                            else:
            ...
            if expr.quantification_op == 'iota':
                ...
                try:
                    ...
                    if not g_options.use_softmax_iota:
                        if g_options.normalize_iota and not self.training:
                            ### normalize so the max value is 1 and min is 0
                            return TensorValue(
                                expr.return_type,
                                value.batch_variables,
                                value.tensor / value.tensor.max(),
                                quantized=False
                            )
                ...
                try:
                    ...
                    return TensorValue(
                        expr.return_type,
                        value.batch_variables,
                        F.softmax(value.tensor, dim=variable_index) if not g_options.normalize else value.tensor,
                        quantized=False
                    )
                finally:
                    del self.variable_stack[expr.variable.name]
