...
class NCVSEConceptEmbedding(VisualConceptEmbeddingBase):
    ...
    def __init__(self, enable_cross_similariy=False, normalize=False):
        super().__init__()
...
class NCLinearConceptEmbedding(nn.Module):
    def __init__(self, tied_attributes: bool = False, normalize: bool = False):
    ...
    def load_extra_state_dict_and_weights(self, state_dict, weights):
        ...
        for i, concept_name in enumerate(state_dict['concepts']):
            if concept_name not in self.concepts:
                print("Concept not found: ", concept_name)
                continue
    ...
    def init_linear_layers(self):
        ...
        if self.tied_attributes:
            ...
            for attribute in self.attributes:                
                self.register_module('attribute_' + str(attribute), nn.Linear(self.linear_input_dim, self.attribute_output_dim))
...
class NCVecConceptEmbedding(nn.Module):
    def __init__(self, embedding_type= "fasttext", tied_attributes: bool = False, normalize: bool = False, obj_feat_dim = 128):
        """Initialize the concept embedding.
        ...
        """
        super().__init__()
        from left.models.concept.word_embedding import FastTextWordEmbedding, RoBERTaWordEmbedding, SpacyWordEmbedding, GloveWordEmbedding, OneHotWordEmbedding
        self.tied_attributes = tied_attributes
        self.attributes = list()
        self.concepts = list()
        self.normalize = normalize
        self.all_concept_embeddings = list()
        self.linear_input_dim = None
        self.linear_output_dim = 0  # Equal to the number of concepts.
        self.attribute_output_dim = None
        self.add_module('linear', None)
        self.add_module('att_linear', None)
        self.object_linear = nn.Linear(obj_feat_dim, 512)

        if embedding_type == "one-hot":
            self.embedder = OneHotWordEmbedding(max_size=128)
        elif embedding_type == "fasttext":
            self.embedder = FastTextWordEmbedding()
        elif embedding_type == "roberta":
            self.embedder = RoBERTaWordEmbedding()
        elif embedding_type == "spacy":
            self.embedder = SpacyWordEmbedding()
        elif embedding_type == "glove":
            self.embedder = GloveWordEmbedding()
        else:
            raise ValueError("Invalid embedding type")
        self.embedding_linear = nn.Linear(self.embedder.embedding_dim, 512)
    def extra_state_dict(self):
        return {'attributes': self.attributes.copy(), 'concepts': self.concepts.copy(), 'linear_input_dim': self.linear_input_dim, 'attribute_output_dim': self.attribute_output_dim}
    ...
    def load_extra_state_dict_and_weights(self, state_dict, weights):
        ...
        #     self.linear.weight.data[self.concept2id[concept_name], :] = weights['linear.weight'][i, :]
        #     self.linear.bias.data[self.concept2id[concept_name]] = weights['linear.bias'][i]
        self.embedding_linear.weight.data = weights['embedding_linear.weight']
        self.embedding_linear.bias.data = weights['embedding_linear.bias']
        self.object_linear.weight.data = weights['object_linear.weight']
        self.object_linear.bias.data = weights['object_linear.bias']
        ### self.comb_linear is a sequential model
        for i in range(0,100,2):
            if f"comb_linear.{i}.weight" in weights:
                self.comb_linear[i].weight.data = weights[f"comb_linear.{i}.weight"]
                self.comb_linear[i].bias.data = weights[f"comb_linear.{i}.bias"]
            else:
                break
        ...
        if g.concept_mapping is not None:
            for c1, c2 in g.concept_mapping.items():
                if f'{c1}_Object' in self.concepts and f'{c2}_Object' in self.concepts:
                    ...
                    self.linear.weight.data[self.concept2id[f'{c1}_Object'], :] = self.linear.weight.data[self.concept2id[f'{c2}_Object'], :]
                    self.linear.bias.data[self.concept2id[f'{c1}_Object']] = self.linear.bias.data[self.concept2id[f'{c2}_Object']]
        if not isinstance(self.all_concept_embeddings, list):
            self.to(self.all_concept_embeddings.device)
    ...
    def init_concept(self, identifier, input_dim):
        self._set_linear_input_dim(input_dim)
        self.concepts.append(identifier)
        identifier = identifier.replace("_Object","")
        embedding = self.embedder.get_embedding(identifier)
        self.all_concept_embeddings.append(embedding)
        self.linear_output_dim += 1
    ...
    def init_linear_layers(self):
        ...
        if self.tied_attributes:
            ...
            for attribute in self.attributes:
                self.register_module('attribute_' + str(attribute), nn.Linear(self.linear_input_dim, self.attribute_output_dim))
        if self.all_concept_embeddings:
            self.all_concept_embeddings = torch.stack(self.all_concept_embeddings).to("cuda")

    def compute_all_similarity(self, query):
        if query.device != self.all_concept_embeddings.device:
            self.to(query.device)
        new_query = query.reshape(-1,query.shape[-1]).unsqueeze(1).repeat(1,self.all_concept_embeddings.shape[0],1)
        new_query = self.object_linear(new_query)

        new_concept_embeddings = self.all_concept_embeddings.unsqueeze(0).repeat(new_query.shape[0],1,1)
        new_concept_embeddings = self.embedding_linear(new_concept_embeddings)

        concat_emb = torch.cat([new_query,new_concept_embeddings],dim=2)
        logits = self.comb_linear(concat_emb)
        logits = logits.reshape(*query.shape[:-1],-1)
        if self.normalize:
            logits = F.sigmoid(logits)
        return logits
    
    def compute_similarity(self, query, concept):
        
        new_query = query.reshape(-1,query.shape[-1]).repeat(1,1)
        new_query = self.object_linear(new_query)

    
        embedding = self.embedder.get_embedding(concept).to(self.all_concept_embeddings.device)

        new_concept_embeddings = embedding.repeat(new_query.shape[0],1)
        new_concept_embeddings = self.embedding_linear(new_concept_embeddings)

        concat_emb = torch.cat([new_query,new_concept_embeddings],dim=1)
        logits = self.comb_linear(concat_emb)
        logits = logits.reshape(*query.shape[:-1],-1)
        if self.normalize:
            logits = F.sigmoid(logits)
