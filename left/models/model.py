...
from left.models.concept.concept_embedding import NCVSEConceptEmbedding, NCLinearConceptEmbedding, NCVecConceptEmbedding
...
class LeftModel(nn.Module):
    ...
    def __init__(self, domain, output_vocab: Optional[Vocab] = None, lm=None):
        super().__init__()
        self._def_configs()
        ...
        if configs.model.concept_embedding == 'vse':
            self.attribute_embedding = NCVSEConceptEmbedding(normalize=configs.model.normalize_scores)
            self.relation_embedding = NCVSEConceptEmbedding(normalize=configs.model.normalize_scores)
            self.multi_relation_embedding = NCVSEConceptEmbedding(normalize=configs.model.normalize_scores)
            ...
            self.attribute_embedding = NCLinearConceptEmbedding(normalize=configs.model.normalize_scores)
            self.relation_embedding = NCLinearConceptEmbedding(normalize=configs.model.normalize_scores)
            self.multi_relation_embedding = NCLinearConceptEmbedding(normalize=configs.model.normalize_scores)
            ...
            self.attribute_embedding = NCLinearConceptEmbedding(tied_attributes=True,normalize=configs.model.normalize_scores)
            self.relation_embedding = NCLinearConceptEmbedding(normalize=configs.model.normalize_scores)
            self.multi_relation_embedding = NCLinearConceptEmbedding(normalize=configs.model.normalize_scores)
            from left.models.reasoning.reasoning import NCOneTimeComputingGrounding
            self.grounding_cls = NCOneTimeComputingGrounding
        ...
        elif configs.model.concept_embedding == 'vec':
            self.attribute_embedding = NCVecConceptEmbedding(configs.model.embedding_type,normalize=configs.model.normalize_scores,obj_feat_dim=configs.model.sg_dims[1] )
            self.relation_embedding = NCVecConceptEmbedding(configs.model.embedding_type, normalize=configs.model.normalize_scores ,obj_feat_dim=configs.model.sg_dims[2])
            self.multi_relation_embedding = NCVecConceptEmbedding(configs.model.embedding_type, normalize=configs.model.normalize_scores, obj_feat_dim=configs.model.sg_dims[3])
            from left.models.reasoning.reasoning import NCOneTimeComputingGrounding
            self.grounding_cls = NCOneTimeComputingGrounding
        else:
        ...
        if configs.model.concept_embedding == 'vec':
            from left.generalized_fol_executor import NCGeneralizedFOLExecutor
            self.parser = NCGeneralizedFOLPythonParser(self.domain, inplace_definition=True, inplace_polymorphic_function=True, inplace_definition_type=True)
        ...
        from left.models.losses import RefExpLoss, AttrClsLoss, QALoss, PickPlaceLoss, QALossV2, RefExpLossV2
        self.refexp_loss = RefExpLoss(add_supervision=configs.train.refexp_add_supervision)
        self.refexp_lossv2 = RefExpLossV2(add_supervision=configs.train.refexp_add_supervision)
        self.attrcls_loss = AttrClsLoss(add_supervision=configs.train.attrcls_add_supervision)
        self.qa_loss = QALoss(output_vocab)
        self.qa_lossv2 = QALossV2(output_vocab)  ### For DK experiments
