    ...
    if args.evaluate_custom is None:
        ...
        if args.validation_data_dir is None:
            ...
            if args.data_retain < 1:
                ...
                train_dataset.unwrapped.retain_data(args.data_retain)
            else:
                train_dataset, validation_dataset = dataset.split_trainval(args.data_tvsplit)
        ...
        dataset = make_dataset(args.evaluate_custom, args.data_scenes_json, args.data_questions_json, args.data_image_root, args.data_output_vocab_json, size= 100 if args.debug else -1)
    ...
    for epoch in range(args.start_epoch + 1, args.epochs + 1):
        ...
        if not args.debug:
            if epoch % args.save_interval == 0:
                ...
                val_metric = float(meters["validation/acc/qa"].avg)
...
def validate_epoch(epoch, trainer, val_dataloader, meters):
    ...
    with tqdm_pbar(total=len(val_dataloader)) as pbar:
        for feed_dict in val_dataloader:
            ...
            if False and run_visualizer and visualizer_index < visualizer_total:
            ...
            pbar.set_description(meters.format_simple(
                ...
                {k: v for k, v in meters.avg.items() if k.startswith('validation') and k.count('/') <= 2},
                compressed=True
            ), refresh=False)
            pbar.update()
